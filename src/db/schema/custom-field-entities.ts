...
export const customFieldDefs = pgTable(
  ...
  (t) => [index("custom_field_defs_workspace_idx").on(t.workspaceId)]
);

export const customFieldVals = pgTable(
  "custom_field_vals",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    workspaceId: text("workspace_id").notNull().references(() => workspaces.id),
  ...
  (t) => [
    index("custom_field_vals_workspace_idx").on(t.workspaceId),
    index("custom_field_vals_entity_idx").on(t.entityId),
  ]
);
