...
import { importSuppliersCsvAction } from "@/actions/inventory";
...
const TEMPLATE = [
  ...
  "شرکت آرتا الکترونیک,رضا محمدی,09121234567,reza@arta-electronic.com,تهران، خیابان ولیعصر,تامین‌کننده اصلی قطعات",
  "فروشگاه پارس کامپیوتر,علی احمدی,09351112233,ali@pars-computer.ir,اصفهان، چهارباغ,فروش لوازم جانبی",
].join("\n");
...
export function ImportSuppliersDialog() {
  ...
  function handleTemplateDownload() {
    ...
    toast.success("قالب ایمپورت تأمین‌کنندگان دانلود شد");
  }
  ...
  function goToPreview() {
    ...
    try {
      ...
      if (raw.length - 1 > MAX_ROWS) {
        setFileError(
          `تعداد ردیف‌ها بیشتر از حد مجاز است (حداکثر ${MAX_ROWS.toLocaleString("fa-IR")} ردیف)`
        );
        return;
      }
  ...
  async function handleImport() {
    setImporting(true);
    setActionError(null);
    const result = await importSuppliersCsvAction(content);
    setImporting(false);
    if (!result.ok) {
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-3xl">
          <DialogHeader>
            <DialogTitle>ایمپورت تأمین‌کنندگان از CSV</DialogTitle>
            <DialogDescription>
              وارد کردن دسته‌جمعی تأمین‌کنندگان از فایل CSV.
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 sm:grid-cols-3">
                    {["نام", "شخص تماس", "موبایل", "ایمیل", "آدرس", "یادداشت"].map(
                      (label) => (
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2 sm:gap-3">
                  <div className="rounded-lg border p-3 text-center">
                    ...
                    <div className="text-xs text-muted-foreground">تأمین‌کننده ایجاد شده</div>
                  </div>
                  <div className="rounded-lg border p-3 text-center">
                    <div className="truncate text-xl font-bold sm:text-2xl">
